...
public class UserController {
	...
	/**
	 * 安卓修改密码
	 *userController.do?changePass
	 * @return
	 */
	@RequestMapping(params = "changePass")
	@ResponseBody
	public JSONObject changePass(HttpServletRequest request,HttpServletResponse response) {
		//获取username password newpassword
		...
		//根据用户名来获取实体
	    String hql0 = "from TSBaseUser where 1=1 AND username = ? ";
    	List<TSBaseUser> tSBaseUserList = systemService.findHql(hql0,username);
    	
    	
    	
    	
    	String passwordString = tSBaseUserList.get(0).getPassword();
    	
		//根据用户名，获取密码
		if (!pString.equals(passwordString)||newpassword=="") {
			jsonObject.put("password", false);
			System.out.println("修改密码失败");
			jsonObject.put("newpassword", false);

		} else {
			
			jsonObject.put("password", true);
			String newpass = PasswordUtil.encrypt(username, newpassword, PasswordUtil.getStaticSalt());
			tSBaseUserList.get(0).setPassword(newpass);
			systemService.saveOrUpdate(tSBaseUserList.get(0));
			jsonObject.put("newpassword", true);
