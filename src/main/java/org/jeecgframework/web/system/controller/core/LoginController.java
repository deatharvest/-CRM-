...
//        update-end--Author:zhangguoming  Date:20140226 for：添加验证码
	 ...
	 * @return
	 */
	@RequestMapping(params = "androidlogin")
	@ResponseBody
	public JSONObject androidlogin(HttpServletRequest request) {
		...
		TSUser u = userService.checkUserExits(user);	//通过用户名密码检查TSUser数据库，返回用户信息或null
		
		if(u == null){
			j.put("msg", "用户名密码错误");
			j.put("success", false);
			System.out.println(username + "安卓设备尝试登陆，并没有成功");
			return j;
		}else{
	       ...
	       System.out.println(ResourceUtil.getSessionUserName());
	       
			//返回登陆信息，并拥有权限
	        j.put("user", u);
			j.put("success", true);
			System.out.println(username +"安卓设备成功登陆");
			return j;
